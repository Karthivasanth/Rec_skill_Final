...
def home_view(request,*args,**kwargs):
    return render(request,'hello.html',{'home':'/login','register':'/register','rec':'/recskill'})
...
def rec(request):
    return render(request,'index.html',{'register':'/register','company':'/company'})
...
def stu_dashboard(request):
    ...
    print(mem)
    return render(request , "stu_dashboard.html",{'name':use,'c': data})
...
def compnotification(request):
    global user
    usearch=Cjob.objects.filter(userid=request.session['member']).values()
    print(usearch)
...
def stut_pro(request, name=None, id=None):
    ...
    if name:
        stss=japp.objects.filter(j_id=id).values_list('sts')[0]
        st=stss[0]
        print(name)
        print(st)
        use = name
...
def usersettings(request):
    global user
    usearch=logindb.objects.get(username=request.session['member'] )
...
def updateusers(request):
    ...
    if request.method == 'POST':
            ...
            logindb.objects.filter(username=request.session['member']).update(username=username,firstname=firstname,email=email,password=password)
            username = request.POST['username']
            firstname = request.POST['firstname']
            email = request.POST['email']
            password = request.POST['password']
